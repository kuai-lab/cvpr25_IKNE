...
from ikne.dataset import build_dataloader, build_dataset
from ikne.evaluation import ScanNetEval
from ikne.utils import AverageMeter, get_root_logger, rle_decode, write_obj
import numpy as np
import wandb
...
def get_model(cfg, model_name):
    if model_name == 'IKNE':
        from ikne.model import IKNE
        model = IKNE(**cfg.model).cuda()
    else:
...
def main():
    ...
    # model
    model_name = cfg.model.pop("name", "IKNE")

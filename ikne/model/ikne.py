...
from ikne.utils import cuda_cast, rle_encode
...
class IKNE(nn.Module):
    ...
    def __init__(
        ...
        self.on_off_loss_weight = 0.001

    def train(self, mode=True):
        super(IKNE, self).train(mode)
    ...
    @cuda_cast
    def loss(self, scan_ids, voxel_coords, p2v_map, v2p_map, spatial_shape, feats, insts, superpoints, coords_float, batch_offsets, batch_points_offsets):
        ...
        on_off_loss = out['on_off_loss'] * self.on_off_loss_weight
        loss += on_off_loss

        loss_dict['loss'] = loss.item()
        loss_dict['on_off_loss'] = on_off_loss

        return loss, loss_dict

    @cuda_cast
    def predict(self, scan_ids, voxel_coords, p2v_map, v2p_map, spatial_shape, feats, insts, superpoints, coords_float,
                batch_offsets, batch_points_offsets):
        batch_size = len(batch_offsets) - 1
        voxel_feats = pointgroup_ops.voxelization(feats, v2p_map)
        input = spconv.SparseConvTensor(voxel_feats, voxel_coords.int(), spatial_shape, batch_size)
    ...
    def predict_by_feat(self, scan_ids, out, superpoints, insts):
        pred_labels = out['labels']     # [B, num queries, num classes]
        pred_masks = out['masks']
        pred_scores = out['scores']

        scores = F.softmax(pred_labels[0], dim=-1)[:, :-1]  # [num queries, num classes - 1]
        ...
        return dict(scan_id=scan_ids[0], pred_instances=pred_instances, gt_instances=gt_instances, cat_prob=pred_labels[0, topk_idx])

...
class QueryDecoder(nn.Module):
    ...
    def forward_iter_pred(self, x, pos, batch_offsets):
        ...
        for i in range(B):
            ...
            inst_feats_batched[:lengths[i], i, :] = inst_feats[start:end]   # [num_sp, bsz, d_model]
        ...
        # ======== Instance-wise Knowledge Aggregation =========
        output = intermediate_results[-1]   # [4, 400, 256]

        total_on_off_loss = 0
        all_sim = output @ output.transpose(1, 2)
        all_sim = (all_sim - all_sim.min()) / (all_sim.max() - all_sim.min())
        for i in range(all_sim.shape[0]):
            on_diag = torch.diagonal(all_sim[i]).add(-1).pow(2).sum()
            diagonal = torch.diag(all_sim[i])
            off_diag = (all_sim[i] - torch.diag(diagonal)).pow(2).sum()

            on_off_loss = on_diag + self.on_off_weight * off_diag
            total_on_off_loss += on_off_loss
